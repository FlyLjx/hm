...
export function taskImages(task) {
  if (task?.resultUrls?.length) return task.resultUrls
  if (task?.resultUrl) return [task.resultUrl]
  if (task?.directResultUrls?.length) return task.directResultUrls
  return task?.directResultUrl ? [task.directResultUrl] : []
}
