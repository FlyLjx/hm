  ...
  setup() {
    ...
    const redeemFields = (row) => row
      ? [
          ...
          { key: 'status', label: '状态', type: 'select', defaultValue: 'active', options: redeemStatusOptions },
    ...
    return { adminApi, redeemFields, redeemStatusOptions, shopFields, subscriptionFields }
  },
  template: `
    ...
    <CrudPage v-else-if="mode === 'redeem'" title="卡密兑换" singular="卡密" description="生成、禁用、删除兑换卡密。" paginated search
      :default-filters="{ status: 'all' }"
      :filters="[{ key: 'status', options: [{ label: '全部卡密', value: 'all' }, ...redeemStatusOptions] }]"
