...
func (r *Repository) ActivityRanking(ctx context.Context, days int, limit int) ([]ActivityRank, error) {
	...
	rows, err := r.db.QueryContext(ctx, `
		...
		ORDER BY success_images DESC, task_count DESC, credits_used DESC, last_active_at DESC
		LIMIT ?
	`, since, limit)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.UserID, &item.UserEmail, &item.UserStatus, &item.TaskCount, &item.SuccessTasks, &item.SuccessImages, &item.CreditsUsed, &lastActive); err != nil {
			return nil, err
		}
		if lastActive.Valid {
			value := lastActive.Time.In(time.Local).Format(time.RFC3339)
			item.LastActiveAt = &value
		}
		item.WindowDays = days
		item.Rank = len(items) + 1
		item.ActivityScore = float64(item.SuccessImages)*10 + float64(item.SuccessTasks)*4 + float64(item.TaskCount)*2 + item.CreditsUsed
