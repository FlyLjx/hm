...
const textOnlyModelNames = new Set(['gpt-5-5', 'gpt5.5', 'gpt-5.5', 'gpt5-5'])
...
export function getActiveModelsByCapability(models, capability = 'chat_image') {
  return models
    ...
    .filter((model) => capability !== 'chat_image' || !isTextOnlyModel(model))
    .sort((a, b) => {
...
function isTextOnlyModel(model) {
  const names = [model?.modelName, model?.displayName].map((item) => String(item || '').trim().toLowerCase())
  return names.some((name) => textOnlyModelNames.has(name))
}
